...
public class AddressableLEDController {
    ...
    /***
     * Initialized the controller. This controller is meant to control individually addressable LEDs. It works for WS2812 architecture, but it should work for WS2812B as well.
     * 
    ...
    /**
     * Starts the notifier handler that will run the custom function. It is recommended to have the period be 0.02, meaning it will run 50 times a second, but any number should work just fine.
     * 
    ...
    /**
     * This is a uniform way to handle most simple custom functions (like a moving rainbow) to control the LEDs. Every time this is called, it raises the step counter by 1.
     * 
     * @param ledStepperFunction custom LED controller function <step number, LEDbuffer, LEDbuffer>
     */
    ...
    /**
     * Same as rainbow() except the rainbow is stretch out over the whole length of the LED strip.
     * 
     * @param stepPeriod number of steps for the pattern to move a whole wavelength
     * @param step the step number, each step will generally be 1/50th of a second [given by handler]
     * @param buffer the LED buffer, it is modified then returned [given by handler]
    ...
    /**
     * And example of a custom LED step function. It is a moving rainbow has a custom wavelength and movement "speed."
     * 
     * @param stepPeriod number of steps for the pattern to move a whole wavelength
     * @param wavelength length (in LEDs) of the wave, so number of LEDS between the two red points (or any other color)
     * @param length length of LED strip
     * @param step the step number, each step will generally be 1/50th of a second [given by handler]
     * @param buffer the LED buffer, it is modified then returned [given by handler]
     * @return the modified LED buffer
     */
    public static AddressableLEDBuffer rainbow(int stepPeriod, int wavelength, int length, int step,  AddressableLEDBuffer buffer) {
        int degrees = 360;
        int saturation = 1;
        int value = 1;
